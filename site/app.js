...
const DETAIL_PREFIX = BASE_PATH + '/regulations/';
...
async function openRegulation(id, trigger) {
  ...
  detailContext = {
    id,
    previousUrl: location.pathname.startsWith(DETAIL_PREFIX) ? (currentViewUrl || BASE_PATH + '/') : (location.pathname + location.search),
...
function closeDetail() {
  ...
  if (ctx) {
    if (location.pathname.startsWith(DETAIL_PREFIX)) history.replaceState(null, '', ctx.previousUrl || BASE_PATH + '/');
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-jurisdiction-jump]').forEach((button) => button.addEventListener('click', () => { navigateTo(BASE_PATH + '/library?jurisdiction=' + encodeURIComponent(button.dataset.jurisdictionJump)); }));
